...
| 主框架 | 动态验证读什么 |
| --- | --- |
| **Next.js** | 增量：读 `nextjs-testing-guide` 中与本轮 §2 相关的章节；全量 §2：完整读取 `.Codex/skills/nextjs-testing-guide/SKILL.md` |
...
- §2 验收标准、§8 验证：仅有证据项 `[x]`
- **失败 / 跳过 / 阻塞**：保持 `[ ]`，在条目下或验证记录写 `❌` / `⚠️` 与原因（见 `.Codex/rules/todo-writeback.md`）
- 默认不改 §3/§4，除非验证中明确覆盖
- 末尾追加/更新 **验证记录**（模板见下）
